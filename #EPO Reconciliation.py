...
def get_new_renewals_dataframe():
    crm_renewals_query = """
        SELECT 
            ...
            -- Renewal year - prioritize service request
            COALESCE(SR.ipo_renewalyear, IPR.ipo_renewalyear, YEAR(I.ipo_datecompleted) - YEAR(IPR.ipo_applicationdate) + 1) AS renewal_year,
            -- Fees
            COALESCE(F.ipo_totalfee, 0.0) AS renewal_fee,
            SR.ipo_prescribedukfee AS prescribed_uk_fee,            
            -- LOR Indicator
            SR.ipo_lorindicator AS lor_indicator,            
            -- Renewal due date - with fallback calculation, renamed to match function expectation
            COALESCE(
                IPR.ipo_nextrenewaldate,
                ADD_MONTHS(IPR.ipo_applicationdate,COALESCE(SR.ipo_renewalyear,IPR.ipo_renewalyear, 
                                    YEAR(I.ipo_datecompleted) - YEAR(IPR.ipo_applicationdate) + 1) * 12)
                ) AS renewal_due_date,            
            -- Processing dates
